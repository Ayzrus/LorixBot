...
new Command({
  name: "setpercentageoftheft",
  description: "[Admin] Choose a % for the amount to be stolen",
  dmPermission: false,
  ...
  options: [
    {
      name: "percentage",
      description:
        "Choose a % for the amount to be stolen",
  ...
  async run(interaction) {
    ...
    guildData.PercentageOfTheft = parseInt(percentage?.toString() ?? "50");

    guildData.save();

    const embed = new EmbedBuilder({
      title: "Info System",
      description: "Percentage has been set successfully.",
      color: 0x006400,
